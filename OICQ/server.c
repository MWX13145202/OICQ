...
void handleClentPrivateChatRequest(int sockfd,PDU *pdu)
{
	if(NULL == pdu){
		return;
	}
...
void handleClentGroupChatRequest(int sockfd,PDU *pdu)
{
	if(NULL == pdu){
		return;
	}
	...
	while(1){
		...
		switch(pdu -> uiMsgType)
		{
			...
			case ENUM_MSG_TYPE_PRIVATE_CHAT_REQUEST:
				handleClentPrivateChatRequest(sockfd,pdu);
				break;
			case ENUM_MSG_TYPE_GROUP_CHAT_REQUEST:
				handleClentGroupChatRequest(sockfd,pdu);
				break;
			default:
				break;
		}
...
int main()
{
	...
	int sockfd = mkSocket(); //产生一个sockfd用于监听客户端连接
